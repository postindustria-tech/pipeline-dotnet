...
namespace FiftyOne.Pipeline.Core.Tests.Data
{
    ...
    public class EvidenceTests
    {
        ...
        [TestMethod]
        public void GetStringValues_NullValue()
        {
            TestSingleValue(null);
        }

        [TestMethod]
        public void GetStringValues_NoMatchingKey()
        {
            var key = "test";
            _evidence[key] = "123";
            var result = _evidence.GetStringValues(key + key);
            Assert.AreEqual(1, result.Count());
            Assert.IsNull(result.ElementAt(0));
        }
        ...
        [TestMethod]
        public void GetStringValues_ObjectListWithNull()
        {
        ...
        [TestMethod]
        public void GetStringValues_StringValuesWithNull()
        {
            var value = new StringValues(
                new string[] {
                    "value1",
        ...
        private void TestMultiValue(IEnumerable value)
        {
            ...
            foreach(var entry in expectedValues)
            {
                Assert.IsTrue(result.Contains(entry), 
                    $"The result is missing the expected value '{entry}'. " +
                    $"Full list of values in result: {string.Join(", ", result)}");
            }
        }
    }
}
